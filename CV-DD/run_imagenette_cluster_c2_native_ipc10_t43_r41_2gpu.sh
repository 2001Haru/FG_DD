...
C10_NATIVE_ROOT="$Main_Data_Path/class_in_class/imagenette_cluster_c10_native_recovery_bs100_c1_labeler"
EXP_ROOT="$Main_Data_Path/class_in_class/imagenette_cluster_c2_native_ipc10_t43_r41"
LOG_ROOT="${LOG_ROOT:-$ROOT/logs/imagenette_cluster_c2_native_ipc10_t43_r41}"
VAL_DIR="$val_dir/imagenet-nette/test"
PARTITION="$CLUSTER_ROOT/data/dinov2_cluster_c2_seed42"
CLUSTER_TEACHER="$CLUSTER_ROOT/tseed43/models/dinov2_cluster_c2_seed42_tseed43/ResNet18.pth"
C1_TEACHER="$C1_ROOT/tseed43/models/random_c1_pseed42_tseed43/ResNet18.pth"
PATCH_ROOT="$EXP_ROOT/patches/cluster_c2_native20_ipc5"
NATIVE_SYN_ROOT="$EXP_ROOT/native_synthetic"
NATIVE_SYN="$NATIVE_SYN_ROOT/cluster_c2_native20_ipc5_rseed41"
COARSE_SOURCE="$EXP_ROOT/coarse_source/cluster_c2_native20_ipc5_rseed41"
FKD_BASE="$EXP_ROOT/fkd/c1_soft_rseed41"
FKD_FINAL="${FKD_BASE}_bs10_ipc10"
RESULT_ROOT="$EXP_ROOT/per_class"
POST_ROOT="$EXP_ROOT/post_eval"
mkdir -p "$EXP_ROOT" "$LOG_ROOT" "$RESULT_ROOT" "$POST_ROOT"

fail(){ echo "Cluster-C2 native IPC10 experiment failed: $*" >&2; exit 1; }
...
partition_valid="$(python -c "import json; q=json.load(open('$PARTITION/hierarchy.json')); m=q.get('fine_to_coarse',{}); print(q.get('kind')=='imagenette_balanced_dinov2_clusters' and int(q.get('subclasses_per_coarse',-1))==2 and len(m)==20 and all(int(m[str(i)])==i//2 for i in range(20)))")"
[[ "$partition_valid" == True ]] || fail "invalid parent-major DINO Cluster C2 hierarchy"
...
if ! (( patch_count == 100 )) || ! python "$ROOT/class_in_class/validate_cvdd_patch_tree.py" \
    --patch-dir "$PATCH_ROOT" --classes 20 --patches-per-class 5 --image-size 224 \
    ...
    python -u "$ROOT/class_in_class/generate_patches.py" \
        --data-dir "$PARTITION" --teacher "$CLUSTER_TEACHER" --teacher-num-classes 20 \
        --teacher-architecture torchvision --num-classes 20 --patches-per-class 5 \
...
echo "[2/6] Native-20 BS20 recovery, five images per subclass"
...
expected="native20:tseed=43:rseed=41:teacher=$(sha256sum "$CLUSTER_TEACHER"|awk '{print $1}'):hierarchy=$(sha256sum "$PARTITION/hierarchy.json"|awk '{print $1}'):patch=$patch_hash:batch=20:native_ipc=5:iter=4000:lr=0.1:r_bn=0.01"
native_count=0
[[ -d "$NATIVE_SYN" ]] && native_count="$(find "$NATIVE_SYN" -type f -name '*.jpg' | wc -l)"
if [[ ! -f "$marker" || "$(tr -d '[:space:]' < "$marker")" != "$expected" || "$native_count" != 100 ]]; then
    ...
    python -u "$ROOT/recover/recover.py" \
        ...
        --dataset-name imagenet-nette --recovery-num-classes 20 --teacher-num-classes 20 \
        --batch-size 20 --syn-data-path "$NATIVE_SYN_ROOT" --patch-dir "$PATCH_ROOT" \
        ...
        --lr 0.1 --iteration 4000 --r-bn 0.01 --store-best-images \
        --ipc-start 0 --ipc-end 5 --initialisation-method Patches --patch-diff medium \
        --seed 41 --skip-completed > "$LOG_ROOT/recover.log" 2>&1 || fail recovery
fi
[[ "$(find "$NATIVE_SYN" -type f -name '*.jpg' | wc -l)" == 100 ]] || fail "native recovery count"
[[ "$(find "$NATIVE_SYN" -mindepth 1 -maxdepth 1 -type d | wc -l)" == 20 ]] || fail "native directory count"

echo "[3/6] Group five images per subclass into coarse IPC10"
python -u "$ROOT/class_in_class/collapse_synthetic_subclass_imagefolder.py" \
    ...
    --output-dir "$COARSE_SOURCE" --native-classes 20 --coarse-classes 10 \
    --images-per-native 5 > "$LOG_ROOT/collapse.log" 2>&1 || fail collapse
[[ "$(find "$COARSE_SOURCE" -type f -name '*.jpg' | wc -l)" == 100 ]] || fail "coarse source count"

echo "[4/6] C1 CutMix FKD relabel at coarse IPC10"
fkd_count=0
[[ -d "$FKD_FINAL" ]] && fkd_count="$(find "$FKD_FINAL" -type f -name 'batch_*.tar' | wc -l)"
if (( fkd_count != 3000 )); then
    if (( fkd_count > 0 )); then
...
[[ "$(find "$FKD_FINAL" -type f -name 'batch_*.tar' | wc -l)" == 3000 ]] || fail "FKD count"
...
post_one(){
    ...
    run_name="cluster_c2_native_ipc10_${label_arm}_t43_r41_s${student_seed}"
    common=(
...
echo "[6/6] Summarize strict C2/C1/C10 comparisons"
python -u "$ROOT/class_in_class/summarize_imagenette_cluster_c2_native_ipc10.py" \
    --experiment-root "$EXP_ROOT" --c1-root "$C1_ROOT" \
    --c10-native-root "$C10_NATIVE_ROOT" --output "$EXP_ROOT/analysis/summary.json" \
