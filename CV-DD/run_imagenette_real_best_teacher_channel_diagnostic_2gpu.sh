...
EXP_ROOT="$Main_Data_Path/class_in_class/imagenette_real_best_teacher_channel_diagnostic"
LOG_ROOT="${LOG_ROOT:-$ROOT/logs/imagenette_real_best_teacher_channel_diagnostic}"
...
collect_one(){
    ...
    python -u "$ROOT/class_in_class/audit_imagenette_best_teacher_channels.py" collect \
        ...
        --c1-training-epoch 16 --c1-temperature 100 \
        --c100-training-epoch 100 --c100-temperature 200 \
        --selection-label "Random Real downstream best" \
        > "$LOG_ROOT/collect_tseed${teacher_seed}.log" 2>&1
}

echo "[1/2] Collect Random-Real-best soft-label matrices on two A100s"
...
python -u "$ROOT/class_in_class/audit_imagenette_best_teacher_channels.py" analyze \
    ...
    --output "$EXP_ROOT/analysis/real_best_teacher_channel_diagnostic.json" \
    > "$LOG_ROOT/analyze.log" 2>&1

echo "Diagnostic complete: $EXP_ROOT/analysis/real_best_teacher_channel_diagnostic.json"
