...
OUTPUT="$BASE/imagenette_temperature_optimized_utility_logit_R_analysis"
LOG_ROOT="${LOG_ROOT:-$ROOT/logs/imagenette_temperature_optimized_utility_logit_R_analysis}"
mkdir -p "$OUTPUT" "$LOG_ROOT"

PYTHONPATH="$ROOT${PYTHONPATH:+:$PYTHONPATH}" \
python -u "$ROOT/class_in_class/analyze_imagenette_temperature_optimized_utility_logit_r.py" \
...
echo "Complete: $OUTPUT/temperature_optimized_utility_logit_R_analysis.json"
